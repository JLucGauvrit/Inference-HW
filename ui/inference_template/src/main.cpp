...
// Random Sampling
int SelectFromLogits(const inference_fpga::Tensor1dLogits& prob_dist) {
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_real_distribution<> dis(0, 1);

  const int vocab_size = inference_fpga::kVocabSize;
  float rand = dis(gen);
...
int main(int argc, char* argv[]) {
  ...
  if (args.help) {
    std::cout << "Usage: " << argv[0] << " [options]" << std::endl
              ...
              << "  --prompt, -p    : Initial prompt string" << std::endl;
    return 0;
  }

  // 2. Print hyper parameters.
  std::cout << "Hyper Parameters" << std::endl
            << "  dim       : " << inference_fpga::kDim << std::endl
            << "  ffn_dim   : " << inference_fpga::kFFNDim << std::endl
            << "  n_layers  : " << inference_fpga::kNumLayers << std::endl
            << "  n_heads   : " << inference_fpga::kNumHeads << std::endl
            << "  n_kv_heads: " << inference_fpga::kNumKVHeads << std::endl
            << "  vocab_size: " << inference_fpga::kVocabSize << std::endl
            << "  seq_len   : " << inference_fpga::kSeqLen << std::endl;
  ...
  static inference_fpga::Weights weights;
  static inference_fpga::Tensor2dTok tok_emb_table; // [vocab_size, dim]
  inference_fpga::LoadWeights(weights, tok_emb_table, weight_fs);
  weight_fs.close();
  ...
  static inference_fpga::Vocab vocab;
  const int vocab_size = inference_fpga::kVocabSize;
  inference_fpga::ResizeVocab(vocab, vocab_size);
  inference_fpga::LoadVocab(vocab, vocab_fs);
  vocab_fs.close();
  ...
  OCL_CHECK(err, err = kernel_matmul.setArg(3, inference_fpga::kDim));
  OCL_CHECK(err, err = kernel_matmul.setArg(4, inference_fpga::kDim));
  ...
  // 6. Decode
  static inference_fpga::Context ctx;
  inference_fpga::Tensor1d ctx_input;
  static inference_fpga::Tensor3dCache ctx_k_cache;
  static inference_fpga::Tensor3dCache ctx_v_cache;
  inference_fpga::Tensor1dLogits ctx_logits;
  inference_fpga::Tensor1d ctx_final_norm;
  ...
  if (!args.prompt_str.empty()) {
      // Encode() transforme la chaîne UTF-8 en ids du tokenizer
      if (!inference_fpga::Encode(vocab, args.prompt_str, prompt_ids)) {
          std::cerr << "Prompt : au moins un token inconnu\n";
          return EXIT_FAILURE;
      }
  } else {
      prompt_ids = { 1 };   // pas de prompt -> on démarre avec BOS (=1)
  }
  ...
  /* On fait passer CHAQUE token du prompt dans le réseau
    pour remplir les caches K/V avant la génération. */
  for (int id : prompt_ids) {
      inference_fpga::CopyTensor1d(ctx_input, tok_emb_table[id]);
      inference_fpga::Decode(id, pos, ctx_input, ctx_k_cache, ctx_v_cache,
      ...
      token = id;          // dernier token injecté
      ++pos;               // on avance la position !
  }

  for (int pos = 0; pos < args.max_seq; ++pos) {
  //for (; pos < args.max_seq; ++pos) {


    // 6-1. Load the context input and decode the next token.
    inference_fpga::CopyTensor1d(ctx_input, tok_emb_table[token]);
    inference_fpga::Decode(token, pos, ctx_input, ctx_k_cache, ctx_v_cache,
                 ctx_final_norm, weights
#ifndef USE_CPU_ONLY
                 ,
                 q, kernel_matmul, kernel_mul, kernel_rmsnorm, kernel_softmax,
    ...
    inference_fpga::MutmulVocab(ctx_logits, ctx_final_norm, tok_emb_table);
    ...
    if (args.temp < 1e-5) {
      next = inference_fpga::Argmax(ctx_logits);
    } else {
      for (int q = 0; q < vocab_size; ++q) {
        ctx_logits[q] /= args.temp;
      }
      inference_fpga::Softmax(ctx_logits, ctx_logits);
    ...
    // Dump the contexts.
    if (args.log) {
      DumpContext("log/" + std::to_string(pos) + "_", ctx, inference_fpga::kNumLayers);
    }
