...
//                      ROM.
// 
//    Revisions:        1/13/11 Updated ROM[16,37] LOCKED bitmask to 16'HFC00
//                      5/20/13 ROM[42]
//                      1/15/14 INCLUDE fixed
//
...
module bd_vid_phy_controller_0_plle2_drp
   #(
      //***********************************************************************
   ...
   // required.
   `include "vid_phy_controller_v2_2_7_plle2_drp_func.v"
   ...
   wire [37:0] i_s1_clkfbout           = pll_count_calc(PROG_CLKFBOUT_MULT, w_s1_clkfbout_phase, 50000); 
   wire [9:0]  i_s1_digital_filt       = pll_filter_lookup(PROG_CLKFBOUT_MULT, S1_BANDWIDTH);
   wire [39:0] i_s1_lock               = pll_lock_lookup(PROG_CLKFBOUT_MULT); 
   wire [37:0] i_s1_divclk_divide      = pll_count_calc(PROG_DIVCLK_DIVIDE, 0, 50000);
   wire [37:0] i_s1_clkout0            = pll_count_calc(PROG_CLKOUT0_DIVIDE, w_s1_clkout0_phase, w_s1_clkout0_duty);
   wire [37:0] i_s1_clkout1            = pll_count_calc(PROG_CLKOUT1_DIVIDE, S1_CLKOUT1_PHASE, S1_CLKOUT1_DUTY);
   wire [37:0] i_s1_clkout2            = pll_count_calc(PROG_CLKOUT2_DIVIDE, S1_CLKOUT2_PHASE, S1_CLKOUT2_DUTY);
   //clkout3 is always /2 of clkout0 for HDMI
   wire [37:0] i_s1_clkout3            = pll_count_calc({const_0, PROG_CLKOUT0_DIVIDE[7:1]}, w_s1_clkout0_phase, w_s1_clkout0_duty);

   // Update ROM with new set of values
   always@(posedge SCLK) begin
      // Store CLKOUT0 divide and phase
      rom[1]  = {7'h08, 16'h1000, i_s1_clkout0[15:0]};
      rom[2]  = {7'h09, 16'hFC00, i_s1_clkout0[31:16]};
      ...
      // Store CLKOUT3 divide and phase
      rom[7]  = {7'h0E, 16'h1000, i_s1_clkout3[15:0]};
      rom[8]  = {7'h0F, 16'hFC00, i_s1_clkout3[31:16]};
      
      
      
      // Store the input divider
      rom[13] = {7'h16, 16'hC000, {2'h0, i_s1_divclk_divide[23:22], i_s1_divclk_divide[11:0]} };
      
      // Store the feedback divide and phase
      rom[14] = {7'h14, 16'h1000, i_s1_clkfbout[15:0]};
      rom[15] = {7'h15, 16'hFC00, i_s1_clkfbout[31:16]};
      
      // Store the lock settings
      rom[16] = {7'h18, 16'hFC00, {6'h00, i_s1_lock[29:20]} };
      rom[17] = {7'h19, 16'h8000, {1'b0 , i_s1_lock[34:30], i_s1_lock[9:0]} };
      rom[18] = {7'h1A, 16'h8000, {1'b0 , i_s1_lock[39:35], i_s1_lock[19:10]} };
