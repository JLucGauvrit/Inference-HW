...
// ==============================================================
`timescale 1ns/1ps
module bd_v_frmbuf_wr_0_0_CTRL_s_axi
#(parameter
    ...
    output wire [63:0]                   frm_buffer,
    output wire [63:0]                   frm_buffer2,
...
// 0x34 : Data signal of frm_buffer
//        bit 31~0 - frm_buffer[63:32] (Read/Write)
// 0x38 : reserved
// 0x3c : Data signal of frm_buffer2
//        bit 31~0 - frm_buffer2[31:0] (Read/Write)
// 0x40 : Data signal of frm_buffer2
//        bit 31~0 - frm_buffer2[63:32] (Read/Write)
// 0x44 : reserved
...
localparam
    ...
    ADDR_FRM_BUFFER_DATA_1   = 7'h34,
    ADDR_FRM_BUFFER_CTRL     = 7'h38,
    ADDR_FRM_BUFFER2_DATA_0  = 7'h3c,
    ADDR_FRM_BUFFER2_DATA_1  = 7'h40,
    ADDR_FRM_BUFFER2_CTRL    = 7'h44,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_frm_buffer = 'b0;
    reg  [63:0]                   int_frm_buffer2 = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_FRM_BUFFER_DATA_1: begin
                    rdata <= int_frm_buffer[63:32];
                end
                ADDR_FRM_BUFFER2_DATA_0: begin
                    rdata <= int_frm_buffer2[31:0];
                end
                ADDR_FRM_BUFFER2_DATA_1: begin
                    rdata <= int_frm_buffer2[63:32];
                end
            endcase
        end
    end
...
// int_frm_buffer[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_frm_buffer[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FRM_BUFFER_DATA_1)
            int_frm_buffer[63:32] <= (WDATA[31:0] & wmask) | (int_frm_buffer[63:32] & ~wmask);
    end
end
...
// int_frm_buffer2[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_frm_buffer2[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FRM_BUFFER2_DATA_1)
            int_frm_buffer2[63:32] <= (WDATA[31:0] & wmask) | (int_frm_buffer2[63:32] & ~wmask);
    end
end
